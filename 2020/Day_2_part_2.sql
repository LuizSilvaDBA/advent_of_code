...
-- t2: break original string in 2 based on ":"
t2 as (
   select 
      col1,
      substr(col1, 1, locate(':', col1) - 1) as pwd_rule, 
      substr(col1, locate(':', col1) + 1 ) as pwd
   from t1
),

-- t3: breaks pwd_rule in 2 based on "space"
t3 as (
   select 
      col1,
      substr(pwd_rule, 1, locate(' ', pwd_rule) - 1) as characters_to_select, 
      substr(pwd_rule, locate(' ', pwd_rule) + 1 ) as search_letter, 
      pwd
   from t2
),

-- t4: breaks characters_to_select in 2 based on "-"
t4 as (
   select 
      col1,
      substr(characters_to_select, 1, locate('-', characters_to_select) - 1) as first_character_position, 
      substr(characters_to_select, locate('-', characters_to_select) + 1 ) as second_character_position, 
      search_letter,
      pwd
   from t3
),

-- t5: count occurrences of "search_letter" in the "pwd"
t5 as (
   select 
      col1,
      ...
      search_letter,
      pwd,
      '^.{' || first_character_position || '}[' || search_letter || ']' as regex,
      regexp_like(pwd, '^.{' || first_character_position || '}[' || search_letter || ']') as letters_found_in_first_position,
      regexp_like(pwd, '^.{' || second_character_position || '}[' || search_letter || ']') as letters_found_in_second_position

   from t4 
)
select * from t5
where (letters_found_in_first_position is true and letters_found_in_second_position is false) or
      (letters_found_in_first_position is false and letters_found_in_second_position is true)
