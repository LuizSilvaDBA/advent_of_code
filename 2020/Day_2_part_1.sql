...
t3 as (
   select 
      substr(pwd_rule, 1, locate(' ', pwd_rule) - 1) as min_max, 
      substr(pwd_rule, locate(' ', pwd_rule) + 1 ) as search_letter, 
      pwd
   from t2
),

-- t4: breaks min_max in 2 based on "-"
t4 as (
   select 
      substr(min_max, 1, locate('-', min_max) - 1) as min_allowed, 
      substr(min_max, locate('-', min_max) + 1 ) as max_allowed, 
      search_letter,
      pwd
   from t3
),
...
t5 as (
   select 
      ...
      search_letter,
      pwd,
      regexp_count(pwd, ((search_letter))) as letters_found
   from t4 
)
select * from t5
where letters_found >= min_allowed and 
      letters_found <= max_allowed
